...
- Registered the S3 methods (`qrfactor.default`, `print.qrfactor`,
  ...
  This was the cause of the CRAN archival.
...
- Replaced retired spatial packages. Shapefiles are now read with
  ...
  instead of `rgdal::readOGR()`, and the `maptools` and `mgraph`
  dependencies were removed. A small compatibility layer bridges the
  `sf`/`sp` object differences.
